...
export default function Filmstrip({ items }: Props) {
  return (
    <Reveal>
      <div className="flex snap-x snap-mandatory gap-5 overflow-x-auto px-6 pb-6 [scrollbar-width:thin] sm:px-[max(1.5rem,calc((100vw-72rem)/2))]">
        {items.map((item) => (
          <div
            key={item.title}
